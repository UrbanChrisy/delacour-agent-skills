...
Standalone procedure for the `scaffold-module-structure` skill. Establishes the colocated **module pattern** under `src/modules/` so future feature code has a consistent home. Each domain (e.g. `auth`, `trade`, `transactions`) owns its own `components/`, `sheets/`, `contexts/`, `hooks/`, and `utils/` subfolders.
...
2. Create `src/modules/AGENTS.md` with **exactly** the following contents:
   ...
   | `contexts/`   | Contexts scoped to this module                  |
   ...
   - Route / entry files should stay thin, import UI from the relevant module.
